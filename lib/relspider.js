...
module.exports = function (options) {
        ...
        function (err, allowed) {
          ...
          if (allowed) {
            findRelations(page, function (err, result) {
              ...
              function ($) {
                return parseRelations.call(this, 'canonical', $, true);
              },
              function ($) {
                return parseRelations.call(this, 'me', $);
              }
            ]);
          } else {
          ...
          if (nodeFrom) {
            ...
            step(
              ...
              function (err) {
                ...
                neo4j.query('START n=node({id}), n2=node(*), n3=node(*) MATCH allShortestPaths(n<-[:me|alias*..40]-n2), allShortestPaths(n2-[:me|alias*..40]->n3) WHERE has(n2.hasWebhooks) RETURN distinct n2 AS node, has(n3.unfetched) AS unfetched', {id : nodeFrom.id}, this);
              },
  ...
  parseRelations = function (rel, $, linksOnly) {
    var authors = $('head > link[rel~="' + rel + '"]' + (linksOnly ? '' : ', a[rel~="' + rel + '"]')),
      result = {},
      relations = {},
      i,
      length,
      ...
      step(
        function initializeParsing() {
          ...
          response.parsers.forEach(function (parser) {
            ...
            process.nextTick(function () {
              try {
                callback(false, parser.call(response, $));
              } catch (e) {
                console.log(e);
                callback(true);
              }
