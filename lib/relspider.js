...
module.exports = function (options) {
  ...
  isGraphComplete = function (node, callback) {
    ...
    neo4j.query(query, {id : node.id}, function (err, results) {
      if (err) {
        ...
        callback(null, results.length === 0 || results[0].unfetched === 0);
      }
    });
  };
  ...
  addNewUnfetchedPage = function (options, callback) {
    ...
    step(
      function () {
        pg.query("INSERT INTO urls (url, added, requested) VALUES ($1, NOW(), " + (requested ? 'NOW()' : 'NULL') + ")", [page], this);
      },
      function (err) {
  ...
  toFetch = function (retryCount) {
    ...
    pg.query("UPDATE urls SET locked = NOW() WHERE url = (SELECT url FROM urls WHERE disallowed = false AND failed < 3 AND node IS NOT NULL AND (completed = false OR refresh = true OR fetched < NOW() - interval '" + parseInt(options.minutesUntilRefresh, 10) + " minutes') AND (locked IS NULL OR locked < NOW() - interval '" + parseInt(options.minutesToLock, 10) + " minutes') ORDER BY completed ASC, refresh DESC, fetched ASC, added ASC LIMIT 1) RETURNING url, completed, requested", [], function (err, result) {
      ...
      step(
        ...
        function (err, throttled) {
          if (!err && !throttled) {
            ...
            pg.query("UPDATE urls SET locked = NOW() - interval '" + parseInt(options.minutesToLock, 10) + " minutes' + interval '" + parseInt(options.secondsToThrottle, 10) + " seconds' WHERE url = $1", [page]);
            removeFromFetchQueue(page);
          }
        },
