...
module.exports = function (options) {
  ...
  toFetch = function (retryCount) {
    ...
    pg.query("UPDATE urls SET fetched = NOW() WHERE url = (SELECT url FROM urls WHERE completed = false AND disallowed = false AND failed < 3 AND node IS NOT NULL AND (fetched IS NULL OR fetched < NOW() - interval '" + parseInt(options.minutesToLock, 10) + " minutes') ORDER BY added ASC LIMIT 1) RETURNING url", [], function (err, result) {
      ...
      step(
        function () {
          if (!result) {
            ...
            result.canonical = u.omit(result.canonical, page);
            result.me = u.omit(result.me, page);

            if (u.isEmpty(result.canonical) && u.isEmpty(result.me)) {
              this();
            } else {
