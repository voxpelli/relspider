...
//TODO: Below is what needs to be done pre-Heroku
//TODO: Better logging
...
request = request.defaults({
  ...
  headers: {
    'User-Agent' : 'RelSpider (http://kodfabrik.se/)'
  }
});
...
module.exports = function (options) {
  ...
  isGraphComplete = function (node, callback) {
    var query = 'START n=node({id}), n2=node(*) MATCH allShortestPaths(n-[:me*..40]->n2) WHERE n2.unfetched! = true RETURN COUNT(n2)';
  ...
  fetchRelatedURLs = function (node, callback) {
    var query = 'START n=node({id}), n2=node(*) MATCH allShortestPaths(n-[:me*..40]->n2) RETURN distinct n2.url AS url, count(n2.failed?) AS failed';
  ...
  getRelated = function (page, webhook, callback) {
    ...
    step(
      ...
      function (err, complete) {
        if (err) {
          console.log('Error when looking for completeness');
          callback(true);
        } else if (complete) {
  ...
  addAndConnectPage = function (foundPage, nodeFrom, callback) {
    step(
      function () {
      ...
      function (err, node) {
        ...
        if (nodeFrom && node) {
          node.createRelationshipFrom(nodeFrom, 'me', {});
  ...
  handleResponse = function (err, page, result) {
    ...
    fetchNodeForPage(page, function (err, nodeFrom) {
      ...
      step(
        function () {
          if (!result || u.isEmpty(result.me)) {
            this();
          } else {
            var group = this.group();
            u.each(result.me, function (data, foundPage) {
              addAndConnectPage(foundPage, nodeFrom, group());
            });
          }
        },
        function () {
  ...
  parseRelations = function (rel, window) {
    var authors = window.document.querySelectorAll('a[rel~="' + rel + '"], head > link[rel~="' + rel + '"]'),
      result = {},
      relations = {},
      i,
      length,
