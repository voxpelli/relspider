...
//TODO: Move to module and make optional
// Temporary performance checking
setInterval((function () {
  var exec = require('child_process').exec;

  return function () {
    console.log('Topping');
    var child = exec(process.platform === 'darwin' ? 'top -l 1 -n 0' : 'top -n 1 -b', function (error, stdout, stderr) {
      if (error !== null) {
        console.log('topping error: ' + error);
      } else {
        stdout.toString().split("\n").forEach(function (row) {
          if (
            row.indexOf('Load Avg') !== -1 || row.indexOf('CPU usage') !== -1 || row.indexOf('PhysMem') !== -1 ||
            row.indexOf('load average') !== -1 || row.indexOf('Cpu(s)') !== -1 || row.indexOf('Mem:') !== -1
          ) {
            console.log(row);
          }
        });
...
}()), 2 * 60000); // Checking every two minutes
...
module.exports = function (options) {
  ...
  if (options.statsmix) {
    statsmixClient = new statsmix.Client();
    statsmixClient.addMetric('Crawl rate', crawlMetric, { track : true });
  }
