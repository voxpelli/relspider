...
refresh = function (req, res) {
  ...
  if (false && req.method !== 'POST') {
    ...
    spider.forceRefresh(req.query.url, function (err) {
      if (err) {
        res.json(400, {error : 'An unexpected error occured'});
      } else {
        res.json(202, 'Request has been registered');
...
process.on('SIGTERM', function () {
  console.log('Shutting down the API-worker gracefully...');
  //TODO: This should ideally wait until the HTTP-server is closed?
  spider.close();
});
